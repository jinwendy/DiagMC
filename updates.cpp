...
//===========================================================
int Delete_H(struct diag *t){
	/*In this update, a random choice id made as to whate type of special vertex must be on the Hartree bubble provided that
	...
	line attached to C id the dummy one.*/
	...
	struct node *eline1, *eline2, *p1;
	eline1 = c->data.v.eout;  //eline coming out from C
	eline2 = c->data.v.ein;   //eline coming into C
	p1 = c->data.v.pend;  //pline connects to C

	//printf("p1:%d\n", p1->data.l.phys.screen);
	if (eline1 == eline2) return 1;
	...
	complex <double> ratio_de = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2)*green_bold(&(t->prf_g),eline)*F0*F0;
...
int Move_P(struct diag *t){
	...
	struct node *plineline2;
	plineline2 = a->data.v.pend;
	...
	//change the plineline2 type from F back to W 
	change_pline_type(plineline2);

	complex <double> ratio_nu = green_bold(&(t->prf_g),eline)*F0*interaction(&(t->prf_w),plineline2);
	...
	if (rand_01() > ratio) { //if probability is too small,remove all the change
		...
		//set plineline2 (connecting A) to F
		//plineline2->data.l.phys.screen = F;

		return 1;
	}

	//read out the momentum of eout and eline2
...
int Commute(struct diag *t){
	...
	struct node *eline1, *eline2, *v1, *v2;

	eline1 = t->s_worm->data.v.eout;  //propagator coming out from S worm
	eline2 = t->t_worm->data.v.eout;  //propagator coming out from T worm

	//spin indexes of two elines
	int s1, s2;
	s1 = eline1->data.l.phys.s;
	s2 = eline2->data.l.phys.s;
	...
	struct node *pline1, *pline2;
	pline1 = t->s_worm->data.v.pend;
	pline2 = t->t_worm->data.v.pend;
	
	//Exclude the possibility to create a detached bubble
	//if (v2 == t->s_worm && pline1->data.l.measure == MEASURE) return 1;
	//if (v1 == t->t_worm && pline2->data.l.measure == MEASURE) return 1;
		
	//check the position of v1 and v2, if they not same, reject update
	if (v1->data.v.phys.r != v2->data.v.phys.r) return 1;

	//caluculate probability denominator, before the update
	complex <double> ratio_de = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2);

	//swaps heads of eline1 and eline2 
	connect_lin(eline1, v2, t->s_worm);
	connect_lin(eline2, v1, t->t_worm);

	complex <double> ratio_nu = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2);
	...
	if (ratio >= 1) ratio = 1;
		...
		if (rand_01() > ratio) { //if probability is too small,remove all the change
			...
			connect_lin(eline2, v2, t->t_worm);
			return 1;
		}
	}
...
int Insert(struct diag *t){
	...
	//find vertex  B
	eline2 = v2->data.v.ein;  //eline coming into V2
	b = eline2->data.l.tail;  //originating vertex of eline2
	...
	connect_lin(new_eline2, v2, new_v2);

	connect_lin(eline1, a, new_v1);
	connect_lin(eline2, new_v2, b);

	//calculate probability numerator, after the update
	complex <double> ratio_nu = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2)*green_bold(&(t->prf_g),new_eline1)*green_bold(&(t->prf_g),new_eline1)*interaction(&(t->prf_w),new_p);
	...
	new_eline2->data.l.phys.p = momentum_add(p2, new_p->data.l.phys.p);

	//change momentum excess
	t->delta = momentum_excess_ver(t->s_worm);

	//if one of eline1,eline2 is the dummy line, the new dummy line has to have the same originating vertex
	//we only consider eline1, because we did not change the originating vertex of line eline2
...
int Remove(struct diag *t){
	...
	struct node *eline1, *eline2, *eline1_move, *eline2_move, *p1;
	...
	eline2 = d->data.v.ein;       //find the eline coming into vertex D
	b = eline2->data.l.tail;      //find vetrex B.
	eline2_move = d->data.v.eout; //find the eline coming out from vertex D
	v2 = eline2_move->data.l.head;//find vertex V2,  which is the head vertex of the eline(eline2_move to removed) coming from C
	...
	if (p1->data.l.measure == MEASURE || eline1_move->data.l.measure == MEASURE || eline2_move->data.l.measure == MEASURE) return 1;
	...
	s2 = eline2->data.l.phys.s;
	s2_m = eline2_move->data.l.phys.s;

	if (s1 != s1_m || s2 != s2_m) return 1;

	//caluculate probability denominator, before the update
	complex <double> ratio_de = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2)*green_bold(&(t->prf_g),eline1_move)*green_bold(&(t->prf_g),eline2_move)*interaction(&(t->prf_w),p1);
	...
	//printf("eline1_move:%d eline2_move:%d eline1:%d eline2:%d p1:%d\n", eline1_move->data.id, eline2_move->data.id, eline1->data.id, eline2->data.id, p1->data.id);
	if (ratio >= 1) ratio = 1;
	else{//if ratio smaller than 1, probability is ratio, need compare it to random number
	...
	delete_lin(t, eline2_move);   //delete eline2_move
...
int Dress(struct diag *t){
	...
	//propose update
	struct node *d, *e, *eline1_new, *eline2_new, *p_new;
	int spin_1, spin_2;

	//read out the spin components
	spin_1 = eline1->data.l.phys.s; //spin on eline1
	spin_2 = eline2->data.l.phys.s; //spin on eline2

	//create two new elines and two new vertices
	eline1_new = create_lin(t, ELECTRON, REGULAR, spin_1);
	eline2_new = create_lin(t, ELECTRON, REGULAR, spin_2);
	d = create_ver(t);
	e = create_ver(t);
	...
	connect_lin(eline2, c, e);
	connect_lin(eline2_new, e, a);
	...
	//caluculate probability numerator, after the update
	complex <double> ratio_nu = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2)*green_bold(&(t->prf_g),eline1_new)*green_bold(&(t->prf_g),eline2_new)*interaction(&(t->prf_w),p_new);
	...
	eline2_new->data.l.phys.p = momentum_subtract(eline2->data.l.phys.p, p_new->data.l.phys.p);
...
int Undress(struct diag *t){
	...
	eline1 = d->data.v.ein;
	eline2 = e->data.v.eout;
	b = eline1->data.l.tail;
	c = eline2->data.l.head;
	...
	if (p_move->data.l.measure == MEASURE || eline1_move->data.l.measure == MEASURE
		|| eline2_move->data.l.measure == MEASURE) return 1;
	...
	//caluculate probability denominator, before the update
	complex <double> ratio_de = green_bold(&(t->prf_g),eline1)*green_bold(&(t->prf_g),eline2)*green_bold(&(t->prf_g),eline1_move)*green_bold(&(t->prf_g),eline2_move)*interaction(&(t->prf_w),p_move);
...
int Move_T(struct diag *t){
	...
	if (general_or_bubble == 1 /*&& d->data.v.type != NO*/)  {//calculate probability when A on a bubble
		...
		ratio = factor*fabs(F0 / F0);

		if (ratio >= 1) ratio = 1;
		else{
			if (rand_01() > ratio) return 1; //probability is too small, do nothing
		}
		...
		complex <double> ratio_deline2 = interaction(&(t->prf_w),p);
		...
		//probability
		ratio = abs(ratio_nu1 / ratio_deline1)*abs(ratio_nu2 / ratio_deline2)*factor;

		if (ratio >= 1) ratio = 1;
		else{
			if (rand_01() > ratio) {//if ratio is too small, remove the update

				a->data.v.phys.t = t_old;//set the time component back to original value
				return 1;
			}
		}
